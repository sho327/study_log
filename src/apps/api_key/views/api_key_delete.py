...
KINO_ID = "api-key-delete"

class ApiKeyDeleteView(BaseAPIView):
    """
    アーティスト削除APIクラス
    Create
        Author: Kato Shogo
    ...
    def delete(self, request, artist_id, *args, **kwargs):
        ...
        """
        try:
            return self.api_key_delete(request, api_key_id, *args, **kwargs)
    ...
    def api_key_delete(self, request, api_key_id, *args, **kwargs):
        """
        APIキー削除処理
        Args:
            request:  HTTPリクエスト
        ...
        log_output_by_msg_id(
            log_id="MSGI004", 
            params=[KINO_ID, f"Deleted ID: {api_key_id}"], 
            logger_name=LOG_METHOD.APPLICATION.value
        )
