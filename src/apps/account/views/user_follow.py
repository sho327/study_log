...
class UserFollowView(BaseAPIView):
    """
    フォローAPIクラス
    Create
        Author: Kato Shogo
    ...
    def post(self, request, target_user_id, *args, **kwargs):
        ...
        """
        try:
            return self.user_follow(request, target_user_id, *args, **kwargs)
    ...
    def user_follow(self, request, target_user_id, *args, **kwargs):
        """
        フォロー処理
        Args:
            request:  HTTPリクエスト
        ...
        # 2. フォロー(サービス実行)
        self.account_service.user_follow(
            date_now, 
            KINO_ID, 
            request.user.id,
            target_user_id,
