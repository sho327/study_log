...
KINO_ID = "log-add-reaction"

class LogAddReactionView(BaseAPIView):
    """
    ログリアクション追加APIクラス
    Create
        Author: Kato Shogo
    ...
    def post(self, request, log_id, emoji_id, *args, **kwargs):
        ...
        """
        try:
            return self.log_add_reaction(request, log_id, emoji_id, *args, **kwargs)
    ...
    def log_add_reaction(self, request, log_id, emoji_id, *args, **kwargs):
        """
        ログリアクション追加処理
        Args:
            request:  HTTPリクエスト
        ...
        # 2. サービス実行(ログリアクション追加)
        self.log_service.add_log_reaction(
            date_now=date_now,
            kino_id=KINO_ID,
            user=request.user,
            log_id=log_id,
