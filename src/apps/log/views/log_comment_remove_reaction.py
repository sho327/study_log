...
KINO_ID = "log-comment-remove-reaction"

class LogCommentRemoveReactionView(BaseAPIView):
    """
    コメントリアクション削除APIクラス
    Create
        Author: Kato Shogo
    ...
    def post(self, request, log_comment_id, emoji_id, *args, **kwargs):
        """
        POSTリクエストを受け付ける。
        Method: POST
        Args:
        ...
        """
        try:
            return self.log_comment_remove_reaction(request, log_comment_id, emoji_id, *args, **kwargs)
    ...
    def log_comment_remove_reaction(self, request, log_comment_id, emoji_id, *args, **kwargs):
        """
        コメントリアクション削除処理
        Args:
            request:  HTTPリクエスト
        ...
        log_output_by_msg_id(
            log_id="MSGI003", 
            params=[KINO_ID, f"log_comment_id: {log_comment_id}, emoji_id: {emoji_id}"], 
            logger_name=LOG_METHOD.APPLICATION.value
        )
        
        # 2. サービス実行(コメントリアクション削除)
        self.log_service.remove_log_comment_reaction(
            date_now=date_now,
            kino_id=KINO_ID,
            user=request.user,
