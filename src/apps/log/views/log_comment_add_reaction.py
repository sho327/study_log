...
KINO_ID = "log-comment-add-reaction"

class LogCommentAddReactionView(BaseAPIView):
    """
    コメントリアクション追加APIクラス
    Create
        Author: Kato Shogo
    ...
    def post(self, request, log_comment_id, emoji_id, *args, **kwargs):
        ...
        """
        try:
            return self.log_comment_add_reaction(request, log_comment_id, emoji_id, *args, **kwargs)
    ...
    def log_comment_add_reaction(self, request, log_comment_id, emoji_id, *args, **kwargs):
        """
        コメントリアクション追加処理
        Args:
            request:  HTTPリクエスト
        ...
        # 2. サービス実行(コメントリアクション追加)
        self.log_service.add_log_comment_reaction(
            date_now=date_now,
            kino_id=KINO_ID,
            user=request.user,
